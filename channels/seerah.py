...
# functions
def clear_chat():
    seerah_messages.delete_many({})

    for widget in messages_frame.winfo_children():
        widget.destroy()

clear_chat_button = Button(seerahPage, text="Clear", font=("Arial", 15), bg="sky blue", fg="black", command=clear_chat)
...
def display_messages():
    ...
    messages = seerah_messages.find().sort('timestamp', pymongo.ASCENDING)
...
msj_button=Button(seerahPage,text="Send",font=("Arial", 15), bg="sky blue", fg="black", command=send_message)
msj_button.place(x=825, y=594)

msj_entry=Entry(seerahPage,width=50, font=("Arial", 15),bd=2, bg="sky blue", fg="black", relief=SUNKEN, justify=LEFT)
msj_entry.place(x=227, y=600)

# Canvas for the messages frame and scrollbar
messages_canvas = Canvas(seerahPage)
messages_canvas.place(x=179, y=230, width=650, height=365)

# Scrollbar for the messages frame
messages_scrollbar = Scrollbar(seerahPage, command=messages_canvas.yview)
...
client = MongoClient('mongodb+srv://noorfatimaafzalbutt:0987654321@cluster0.qbhkxkc.mongodb.net/') 
db = client['Accord']
seerah_collection = db['seerah messages']
users_collection = db['users']

# Fetch messages from the 'seerah messages' collection
all_messages = seerah_collection.find({})
