...
hadith_messages = db['hadith messages']
...
hadithPage.title("Ask about Hadith")
...
# functions 
def clear_chat():
    hadith_messages.delete_many({})

    for widget in messages_frame.winfo_children():
        widget.destroy()

clear_chat_button = Button(hadithPage, text="Clear", font=("Arial", 15), bg="sky blue", fg="black", command=clear_chat)
...
def display_messages():
    ...
    messages = hadith_messages.find().sort('timestamp', pymongo.ASCENDING)
...
msj_entry=Entry(hadithPage,width=50, font=("Arial", 15),bd=2, bg="sky blue", fg="black", relief=SUNKEN, justify=LEFT)
msj_entry.place(x=227, y=600)

# Canvas for the messages frame and scrollbar
messages_canvas = Canvas(hadithPage)
messages_canvas.place(x=179, y=200, width=650, height=375)

# Scrollbar for the messages frame
messages_scrollbar = Scrollbar(hadithPage, command=messages_canvas.yview)
...
# client = MongoClient('mongodb+srv://noorfatimaafzalbutt:0987654321@cluster0.qbhkxkc.mongodb.net/') 
# db = client['Accord']
# hadith_collection = db['hadith messages']
# users_collection = db['users']

# # Fetch messages from the 'hadith messages' collection
# all_messages = hadith_collection.find({})
