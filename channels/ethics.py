...
ethics_messages = db['ethics messages']
...
ethicsPage.iconbitmap(logo_path)
ethicsPage.title("Ask about Ethics")
...
# functions 
def clear_chat():
    ethics_messages.delete_many({})

    for widget in messages_frame.winfo_children():
        widget.destroy()

clear_chat_button = Button(ethicsPage, text="Clear", font=("Arial", 15), bg="sky blue", fg="black", command=clear_chat)
...
def display_messages():
    ...
    messages = ethics_messages.find().sort('timestamp', pymongo.ASCENDING)
...
namaz_frame = ttk.Frame(ethicsPage, style="RoundedFrame.TFrame")
...
msj_entry=Entry(ethicsPage,width=50, font=("Arial", 15),bd=2, bg="sky blue", fg="black", relief=SUNKEN, justify=LEFT)
...
client = MongoClient('mongodb+srv://noorfatimaafzalbutt:0987654321@cluster0.qbhkxkc.mongodb.net/') 
db = client['Accord']
ethics_collection = db['ethics messages']
users_collection = db['users']

# Fetch messages from the 'ethics messages' collection
all_messages = ethics_collection.find({})
