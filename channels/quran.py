...
# functions
def clear_chat():
    quran_messages.delete_many({})

    for widget in messages_frame.winfo_children():
        widget.destroy()

clear_chat_button = Button(quranPage, text="Clear", font=("Arial", 15), bg="sky blue", fg="black", command=clear_chat)
...
def display_messages():
    ...
    messages = quran_messages.find().sort('timestamp', pymongo.ASCENDING)
...
msj_entry=Entry(quranPage,width=50, font=("Arial", 15),bd=2, bg="sky blue", fg="black", relief=SUNKEN, justify=LEFT)
...
# back button
back_button=Button(quranPage,text="Back",font=("Arial", 15), bg="sky blue", fg="black",command=go_back)
...
client = MongoClient('mongodb+srv://noorfatimaafzalbutt:0987654321@cluster0.qbhkxkc.mongodb.net/') 
db = client['Accord']
quran_collection = db['quran messages']
users_collection = db['users']

# Fetch messages from the 'quran messages' collection
all_messages = quran_collection.find({})
