...
# functions
def clear_chat():
    roza_messages.delete_many({})

    for widget in messages_frame.winfo_children():
        widget.destroy()

clear_chat_button = Button(rozaPage, text="Clear", font=("Arial", 15), bg="sky blue", fg="black", command=clear_chat)
...
def display_messages():
    ...
    messages = roza_messages.find().sort('timestamp', pymongo.ASCENDING)
...
msj_entry=Entry(rozaPage,width=50, font=("Arial", 15),bd=2, bg="sky blue", fg="black", relief=SUNKEN, justify=LEFT)
...
client = MongoClient('mongodb+srv://noorfatimaafzalbutt:0987654321@cluster0.qbhkxkc.mongodb.net/') 
db = client['Accord']
roza_collection = db['roza messages']
users_collection = db['users']

# Fetch messages from the 'roza messages' collection
all_messages = roza_collection.find({})
