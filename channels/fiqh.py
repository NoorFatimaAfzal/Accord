...
# functions 
def clear_chat():
    fiqh_messages.delete_many({})

    for widget in messages_frame.winfo_children():
        widget.destroy()

clear_chat_button = Button(fiqhPage, text="Clear", font=("Arial", 15), bg="sky blue", fg="black", command=clear_chat)
...
def display_messages():
    ...
    messages = fiqh_messages.find().sort('timestamp', pymongo.ASCENDING)
...
client = MongoClient('mongodb+srv://noorfatimaafzalbutt:0987654321@cluster0.qbhkxkc.mongodb.net/') 
db = client['Accord']
fiqh_collection = db['fiqh messages']
users_collection = db['users']

# Fetch messages from the 'fiqh messages' collection
all_messages = fiqh_collection.find({})
